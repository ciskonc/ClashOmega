...
- **默认布局调整**：代理模式切换移至域名检测上方（高频操作置首），LAYOUT_VERSION 升至 3
- **README 补充前置条件**：配置 Clash API 前需先在 Clash Verge Rev 中开启「外部控制」
- **CHANGELOG 清理**：移除 Fork 上游引用和致谢段落，简化旧版本记录
...
- **现象**：未安装 Native Host 时，系统代理错误显示为「跟随系统」
- **修复**：`getSystemProxyStatus()` 新增 `nativeHostAvailable` 字段，`renderSystemProxyStatus()` 检测到 `nativeHostAvailable === false` 时显示「未安装 Native Host」

#### 1.2 自动检测配置路径未填入文本框

- **现象**：自动检测到的配置路径只显示在状态中，未填入文本框
- **修复**：新增 `autoDetectConfigPath()` 函数，当 `clashConfigPath` 为空时自动填入检测到的路径

#### 1.3 `useScriptRule` 逻辑反转

- **现象**：勾选「域名写入订阅文件Yaml」后，规则仍写入 Script.js
- **修复**：`useScriptRule=true`（写入 YAML）时传 `useScript=false` 给 Native Host；`useScriptRule=false`（写入 Script.js，默认）时传 `useScript=true`

### 二、UI 优化

#### 2.1 脚本规则卡片布局

- 额外脚本规则从三行布局改为卡片式两行布局：
  - 第一行：来源标签（JS/YA）+ 规则类型 + 代理组名 + 最终策略
  - 第二行：域名 + 删除按钮
- 卡片带边框、圆角、背景色，视觉上清晰分组

#### 2.2 Native Host 状态检测

- 设置页打开时自动检测 Native Host 安装状态
- 已安装：绿色状态点 + 「Native Host 已安装」
- 未安装：红色状态点 + 安装引导链接

#### 2.3 域名写入模式文案更新

- 复选框文案改为「域名写入订阅文件Yaml」
- 提示文案：「默认写入Script.js，勾选后写入当前订阅的Yaml配置文件」
- 默认不勾选（写入 Script.js）
...
| 文件 | 说明 |
|------|------|
| `native-host/uninstall.ps1` | Native Host 卸载脚本（UTF-8 BOM + 执行策略检测 + Unicode UTF-8 区域兼容） |
| `native-host/uninstall.bat` | BAT 启动入口（绕过 PowerShell 执行策略） |
| `native-host/install.bat` | 安装脚本 BAT 启动入口（绕过 PowerShell 执行策略） |

### 四、i18n 更新

- `system_proxy_null`：`Null` → `未安装 Native Host` / `Native Host Not Installed` / `Native Host 未インストール`
- 新增 Native Host 状态检测相关 i18n key
