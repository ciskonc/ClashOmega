...
function createScriptRuleItem(ruleStr, source, proxies) {
  ...
  div.appendChild(el('span', { className: 'rule-type-tag' }, type));
  div.appendChild(el('span', { className: 'rule-payload', title: fullRule }, payload || '—'));
  div.appendChild(el('span', { className: 'rule-group-name', title: proxy }, proxy));
  div.appendChild(el('span', { className: `rule-policy ${finalProxy.class}`, title: finalProxy.name }, finalProxy.name));
...
function bindQuickAddRule(domain) {
  ...
  document.getElementById('quick-add-btn').addEventListener('click', async () => {
    ...
    if (result && result.success) {
      ...
      const sourceTag = el('span', {
        className: useScript ? 'rule-source-tag rule-source--js' : 'rule-source-tag rule-source--yaml',
        title: useScript ? I18N.t('rule_source_js') : I18N.t('rule_source_yaml')
      }, useScript ? 'JS' : 'YA');
      div.appendChild(sourceTag);

      div.appendChild(el('span', { className: 'rule-type-tag' }, ruleType));
      div.appendChild(el('span', { className: 'rule-payload', title: rule }, inputDomain));
      div.appendChild(el('span', { className: 'rule-group-name', title: policy }, policy));
      div.appendChild(el('span', { className: `rule-policy ${policyClass}` }, policy));

      const btn = el('button', {
        className: 'rule-delete-btn',
        title: I18N.t('rule_delete'),
        dataset: { rule: rule, script: useScript ? '1' : '0' }
      }, '✕');
      btn.setAttribute('data-i18n-title', 'rule_delete');
      div.appendChild(btn);
